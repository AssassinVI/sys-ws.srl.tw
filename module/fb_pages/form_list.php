...
<div class="wrapper wrapper-content animated fadeInRight">
	<div class="col-lg-12">
       ...
       <!-- <button id="sort_btn" type="button" class="btn btn-default">
		...
		<select id="sel_form" class="form-control">
			<option value="">全部</option>
			
		</select>

		<a id="output_excel_btn" class="btn btn-primary" href="javascript:;"><i class="fa fa-file-excel-o"></i> 匯出Excel檔</a>
			...
			<div class="panel-body">
				<div class="table-responsive">
					<table id="table_id_example" class="table responsive">
						<thead>
							<tr>
								<th width="150">表單時間</th>
								<th width="200">表單名稱</th>
								<th width="100">建案</th>
								<th width="100">姓名</th>
								<th >聯絡電話</th>
								
								<th class="none">表單內容</th>
							</tr>
						</thead>
						<tbody>
		...
		$.ajax({
			...
			success: function (data) {
				...
				data.data.forEach(item => {
					$('#sel_form').append(`<option value="${item.form_id}">${item.form_name} (${item.total}張)</option>`);
				});
			}
		});

		//-- 表單選擇 --
		$('#sel_form').change(function (e) { 
			e.preventDefault();
			ajax_data.form_id=$(this).val();
			table.draw();
		});


		//-- 匯出Excel --
		$('#output_excel_btn').click(function (e) { 
			e.preventDefault();
			let form_id_get = $('#sel_form').val()=='' ? '' : `&form_id=${$('#sel_form').val()}`;
			location.href=`outputExcel.php?pageId=${get_url.searchParams.get('pageId')}${form_id_get}`;
		});
	});
</script>
