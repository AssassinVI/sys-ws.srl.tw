           ...
           for ($i=0; $i <count($otr_file)-1 ; $i++) { //比對 
              if ($otr_file[$i]!=$_POST['show_img']) {
               $new_file.=$otr_file[$i].',';
                ...
                unlink('https://'.WEB_HOST.'/product_html/'.$_POST['case_id'].'/img/'.$_POST['show_img']);
              }
           }
           ...
           for ($i=0; $i <count($otr_file)-1 ; $i++) { //比對 
              if ($otr_file[$i]!=$_POST['show_img_ph']) {
               $new_file.=$otr_file[$i].',';
                ...
                unlink('https://'.WEB_HOST.'/product_html/'.$_POST['case_id'].'/img/'.$_POST['show_img_ph']);
              }
           }
        ...
        for ($i=0; $i <count($_FILES['show_img']['name']) ; $i++) { 
            ...
            if(!empty($_FILES['show_img']['name'][$i])){
               ...
               more_fire_upload('show_img', $i, $Tb_index.'_slider_'.($file_num+$i).'.'.$type[count($type)-1], $_GET['Tb_index']);
                ...
                unlink('https://'.WEB_HOST.'/product_html/'.$_GET['Tb_index'].'/img/'.$_POST['old_file'][$i]);
            }
            else{
        ...
        for ($i=0; $i <count($_FILES['show_img_ph']['name']) ; $i++) { 
            ...
            if(!empty($_FILES['show_img_ph']['name'][$i])){
               ...
               more_fire_upload('show_img_ph', $i, $Tb_index.'_slider-ph_'.($file_num+$i).'.'.$type[count($type)-1], $_GET['Tb_index']);
                ...
                unlink('https://'.WEB_HOST.'/product_html/'.$_GET['Tb_index'].'/img/'.$_POST['old_file_ph'][$i]);
            }
            else{
...
<div class="wrapper wrapper-content animated fadeInRight">
	...
	<div class="row">
		<div class="col-lg-12">
            ...
            <div class="form-group">
              <label class="col-sm-2 control-label" for="slide_type"">輪播類型</label>
              <div class="col-sm-6">
                <label >
                 <input type="radio" name="slide_type" value="0" <?php echo $check=$row['slide_type']==0 || empty($row['slide_type']) ? 'checked' : ''; ?> > 標準
                </label>
                ｜
                <label >
                 <input type="radio" name="slide_type" value="1" <?php echo $check=$row['slide_type']==1 ? 'checked' : ''; ?> > 小圖切換
                </label>
                ｜
                <label >
                 <input type="radio" name="slide_type" value="2" <?php echo $check=$row['slide_type']==2 ? 'checked' : ''; ?> > 自訂義
                </label>
              </div>
            </div>
                                  ...
                                  $show_img=explode(',', $row['show_img']);
                                   for ($i=0; $i <count($show_img)-1 ; $i++) { 
                                     $img_txt='<li class="oneFile_div">
                                                   ...
                                                   <div class="old_file" style="background: url(https://'.WEB_HOST.'/product_html/'.$_GET['Tb_index'].'/img/'.$show_img[$i].') center; background-size: cover;"><p>目前圖檔</p> </div>
                                  ...
                                  $show_img_ph=explode(',', $row['show_img_ph']);
                                   for ($i=0; $i <count($show_img_ph)-1 ; $i++) { 
                                     $img_txt='<li class="oneFile_div">
                                                   ...
                                                   <div class="old_file" style="background: url(https://'.WEB_HOST.'/product_html/'.$_GET['Tb_index'].'/img/'.$show_img_ph[$i].') center; background-size: cover;"><p>目前圖檔</p> </div>
