...
<style type="text/css">
	...
	@media (max-width:420px){
	  ...
	  .case_tb_div thead{display: none;}
					...
					<table class="table no-margin">
						...
						<?php $i=1; while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {
							...
							if($_SESSION['admin_per']=='admin' || in_array($row['Tb_index'], $_SESSION['group_case'])){
							  ...
							  switch ($row['OnLineOrNot']) {
								  case '1':
									$OnLineOrNot='<span class="label">啟用</span>';
								  break;
								  case '0':
									$OnLineOrNot='<span class="label label-danger">停用</span>';
								  break;
								  case '-1':
									$OnLineOrNot='<span class="label label-warning">下架</span>';
								  break;
							  }
