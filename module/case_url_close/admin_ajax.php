 ...
 if ($_POST) {
   if ($_POST['type']=='company') {
     ...
     if ($_POST['com_id']=='all') {
       ...
       $sql=$pdo->prepare("SELECT Tb_index, aTitle, version, OnLineOrNot, OrderBy FROM build_case WHERE com_id=:com_id AND OnLineOrNot=-1 ".$where." ORDER BY OnLineOrNot DESC, OrderBy DESC, Tb_index DESC");
