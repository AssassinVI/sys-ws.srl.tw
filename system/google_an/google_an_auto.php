...
$analytics = initializeAnalytics();
  ...
  $pdo=pdo_conn();
  $sql=$pdo->prepare("SELECT google_view_code, Tb_index FROM build_case WHERE google_view_code!='' AND is_auto_an=1 AND OnLineOrNot!=-1");
  $sql->execute();
  while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {
    ...
    //-- (新分析) --
    an_user($row['google_view_code'], $analytics, $row['Tb_index']);

    an_userType($row['google_view_code'], $analytics, $row['Tb_index']);
    an_userCount($row['google_view_code'], $analytics, $row['Tb_index']);
    an_dayLast($row['google_view_code'], $analytics, $row['Tb_index']);
    an_src_time($row['google_view_code'], $analytics, $row['Tb_index']);

    an_bounceRate($row['google_view_code'], $analytics, $row['Tb_index']);

    an_years($row['google_view_code'], $analytics, $row['Tb_index']);
    an_sex($row['google_view_code'], $analytics, $row['Tb_index']);
    an_city($row['google_view_code'], $analytics, $row['Tb_index']);
    an_event($row['google_view_code'], $analytics, $row['Tb_index']);
...
//-- 使用者 --
function an_user($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions');
...
//-- 回訪次數-人數 --
function an_userCount($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'sessionCount');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 上次訪問網站以來經過的天數 --
function an_dayLast($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'daysSinceLastSession');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 使用者年齡 --
function an_years($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'userAgeBracket');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 性別 --
function an_sex($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'userGender');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//--  城市 --
function an_city($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'region');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 功能鈕 --
function an_event($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'uniqueEvents', 'eventCategory');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 跳出率 --
function an_bounceRate($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'bounceRate');
  $result=printResults($response);
...
//-- 使用裝置 --
function an_media($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'deviceCategory');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 流量來源 --
function an_src($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'sessions', 'sourceMedium');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 流量來源(停留時間) --
function an_src_time($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'avgSessionDuration', 'sourceMedium');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
...
//-- 年齡停留時間 --
function an_timeOnSite($VIEW_ID, $analytics, $case_id)
{
  $all_day=date('Y-m-d',strtotime('-1 day'));
  $response = getReport($VIEW_ID, $analytics, $all_day, $all_day, 'avgTimeOnPage', 'userAgeBracket');
  $result=printResults($response);
  $result=explode('|', $result);
  $type=explode(',', $result[0]);
  $one_data=explode(',', $result[1]);
