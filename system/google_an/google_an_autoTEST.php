...
print_r(Analytics_one( '190594922', $analytics));

// $pdo=pdo_conn();
//   $sql=$pdo->prepare("SELECT google_view_code, Tb_index FROM build_case WHERE google_view_code!='' AND is_auto_an=1");
//   $sql->execute();
...
function getReport($VIEW_ID, $analytics, $StartDate, $Metrics, $Dimensions='', $EndDate='today') {
...
function Analytics_one($VIEW_ID, $analytics)
{
  $param=[];

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions');
  $param['one_user']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions', 'userGender');
  $param['one_sex']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions', 'userAgeBracket');
  $param['one_years']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions', 'deviceCategory');
  $param['one_media']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions', 'eventCategory');
  $param['one_event']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions', 'sourceMedium');
  $param['one_src']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'sessions', 'region');
  $param['one_city']=printResults($response);

  $response = getReport($VIEW_ID, $analytics, '2019-04-04', 'avgTimeOnPage', 'userAgeBracket');
  $param['one_timeOnSite']=printResults($response);
